...
// Web 版拿不到本地文件系统，只能在界面上把服务端的路径展示出来
const configPathHint = computed<string>(() => {
  const dataDir = store.serverInfo?.dataDir
  return dataDir === undefined ? '加载中...' : `${dataDir}/config.json`
})

const downloadDirHint = computed<string>(() => store.serverInfo?.downloadDir ?? '加载中...')
